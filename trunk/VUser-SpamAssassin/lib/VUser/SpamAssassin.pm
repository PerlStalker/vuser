...
# $Id: SpamAssassin.pm,v 1.3 2005-12-06 16:28:27 perlstalker Exp $
...
my $c_sec = 'Extension_SpamAssassin';
...
#    return unless VUser::ExtLib::check_bool($cfg{$c_sec}{enable});

    # Connect to the database(s)
    my $scores_dsn = VUser::ExtLib::strip_ws($cfg{$c_sec}{user_scores_dsn});

    if ($scores_dsn) {
	require DBI;
	my $scores_user = VUser::ExtLib::strip_ws($cfg{$c_sec}{user_scores_username});
	my $scores_pass = VUser::ExtLib::strip_ws($cfg{$c_sec}{user_scores_password});
	$dbs{scores} = DBI->connect($scores_dsn, $scores_user, $scores_pass);
	die "Unable connect to database: ".DBI->errstr."\n" unless $dbs{scores};
    }

    my $awl_dsn = VUser::ExtLib::strip_ws($cfg{$c_sec}{user_awl_dsn});
    if ($awl_dsn) {
	require DBI;
	my $awl_user = VUser::ExtLib::strip_ws($cfg{$c_sec}{user_awl_username});
	my $awl_pass = VUser::ExtLib::strip_ws($cfg{$c_sec}{user_awl_password});
	$dbs{awl} = DBI->connect($awl_dsn, $awl_user, $awl_pass);
	die "Unable connect to database: ".DBI->errstr."\n" unless $dbs{awl};
    }

    my $bayes_dsn = VUser::ExtLib::strip_ws($cfg{$c_sec}{bayes_dsn});
    if ($bayes_dsn) {
	require DBI;
	my $bayes_user = VUser::ExtLib::strip_ws($cfg{$c_sec}{bayes_username});
	my $bayes_pass = VUser::ExtLib::strip_ws($cfg{$c_sec}{bayes_password});
...
sub sa_add
{
    ...
    if ($dbs{scores}) {
	my $table = VUser::ExtLib::strip_ws($cfg->{$c_sec}{user_scores_table});
...
sub sa_delall
{
    ...
    if ($dbs{awl}) {
	my $table = VUser::ExtLib::strip_ws($cfg->{$c_sec}{user_awl_table});
	my $sql = "delete from $table where username=?";
	my $sth = $dbs{scores}->prepare($sql)
	    or die "Database error: ".$dbs{scores}->errstr."\n";
	$sth->execute($user)
